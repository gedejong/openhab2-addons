...
import org.openhab.binding.dsmr.device.cosem.CosemObjectFactory;
import org.openhab.binding.dsmr.device.p1telegram.P1TelegramListener.TelegramState;
...
public class P1TelegramParser {
    ...
    public P1TelegramParser(boolean lenientMode, P1TelegramListener telegramListener) {
        ...
        if (telegramListener == null) {
            logger.error("BUG: telegramListener is null. Binding is not working correctly");
        }
    ...
    private void handleUnexpectedCharacter(char c) {
        if (!lenientMode) {
            logger.warn("Unexpected character '{}' in state: {}. This P1 telegram is marked as failed", c, state);
        } else {
            // Communication errors are expected, don't flood the log
    ...
    private void clearInternalData() {
        logger.debug("obisId: {}, cosumObjectValues: {}, crc: {}", obisId.length(), cosemObjectValuesString.length(),
                crcValue.length());
        obisId.setLength(0);
    ...
    private void clearObisData() {
        logger.debug("obisId: {}, cosumObjectValues: {}", obisId.length(), cosemObjectValuesString.length());
        obisId.setLength(0);
        cosemObjectValuesString.setLength(0);
    }
